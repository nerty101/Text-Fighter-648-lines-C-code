...
{
printf("\nYour attack succeeded, enemy attack failed, enemy damaged!\n");
...
{
normag = 0;
dice5 = (rand() % 27) + 1; //WAS 14 then was 17 THEN 18 then 19 NOW 20 OR 21??? 22 23 24 26
...
{
printf("\nYour special attack succeeded, enemy attack failed, enemy badly damaged!\n");
...
{
printf("\nYour Magic Special Attack succeeded, enemy attack failed, enemy badly damaged!\n");
...
if (diflev > 1)
{
enemyhp = ((enemyhp - 5) * (3)) - ((diflev - 1) * (10));
playerhp = (playerhp  * 2) - ((diflev - 1) * ((diflev + 1) * (diflev - 1)));
if (diflev > 7)
playerhp = (playerhp  * 2) - ((diflev - 4) * (diflev - 5));
}
if (contest != 8)
